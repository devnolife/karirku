    ...
    <div className="act-rise space-y-6">
      ...
      <div className="act-card-2 overflow-hidden">
        <div className="hidden grid-cols-12 gap-3 border-b border-[rgba(15,23,42,0.07)] px-5 py-3 md:grid">
          ...
          <Th className="col-span-3 text-right">Auto-Apply</Th>
        </div>
          ...
          {users.map((u) => (
            <li key={u.id} className="act-rowhover grid grid-cols-12 items-center gap-3 px-5 py-4">
              ...
              <div className="col-span-12 flex items-center justify-end gap-2 md:col-span-3">
                ...
                <form action={toggleAutoApplyEntitlementAction.bind(null, u.id)}>
                  <button type="submit" className="text-xs font-semibold text-[var(--act-blue)] hover:underline">
                    {u.autoApply ? "Cabut" : "Aktifkan"}
                  </button>
                </form>
              </div>
            </li>
