...
async function saveGoalAction(formData: FormData) {
  ...
  await saveActiveGoal(user.id, {
    targetRole,
    targetTrack:
      (String(formData.get("targetTrack") ?? "fulltime") as GoalTrack) ||
      "fulltime",
...
function TrackPicker({ defaultValue }: { defaultValue: GoalTrack }) {
  const options: { value: GoalTrack; title: string; desc: string }[] = [
