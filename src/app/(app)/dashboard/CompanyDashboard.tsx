  ...
  return (
    <div className="act-rise mx-auto max-w-[1200px] space-y-8 px-6 py-8 md:px-10 lg:py-10">
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-12 lg:items-stretch lg:gap-8">
        <div className="flex flex-col justify-center lg:col-span-7 lg:py-5">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              ...
              className="group inline-flex items-center gap-2 rounded-full bg-brand-950 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-600"
            >
              Posting lowongan
            ...
            <Link
              ...
              className="inline-flex items-center rounded-full border border-[rgba(4,39,24,0.14)] bg-white px-5 py-3 text-sm font-semibold text-brand-950 transition-colors hover:bg-brand-50"
            >
              Lihat kandidat
            </Link>
          </div>
        ...
        <aside className="rounded-[24px] border border-[rgba(4,39,24,0.08)] bg-[var(--act-wash-petal)] p-6 sm:p-7 lg:col-span-5">
          <span className="act-eyebrow text-brand-950">Candidate health</span>
          <div className="act-display mt-3 text-5xl tracking-[-0.05em] text-brand-950">
          ...
          <div className="mt-6 grid grid-cols-2 border-y border-[rgba(4,39,24,0.12)] py-4">
            <div className="border-r border-[rgba(4,39,24,0.12)] pr-4">
              <div className="act-display text-2xl text-brand-950">{c.interviews}</div>
              <div className="mt-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgba(4,39,24,0.62)]">Interview</div>
            </div>
            <div className="pl-4">
              <div className="act-display text-2xl text-brand-950">{c.offers}</div>
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-5 lg:gap-6">
        <div className="rounded-[24px] border border-[rgba(4,39,24,0.08)] bg-brand-50 p-5 sm:p-6 lg:col-span-3">
          <div className="flex items-end justify-between gap-4">
            <div>
              <span className="act-eyebrow text-[var(--act-blue)]">Hiring pulse</span>
              <h2 className="act-display mt-2 text-2xl tracking-[-0.035em] text-brand-950">ATS pipeline</h2>
            </div>
            <Link href="/company/candidates" className="mb-0.5 text-xs font-semibold text-brand-600 hover:underline">
              Lihat detail
            </Link>
          </div>
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {CANDIDATE_STAGES.map((st, index) => {
              ...
              const stageClass = ["bg-blue-600/10", "bg-violet-600/10", "bg-amber-500/10", "bg-brand-100"][index];
              return (
                <div key={st.key} className={`min-h-[116px] rounded-[18px] border border-[rgba(4,39,24,0.08)] p-4 ${stageClass}`}>
                  ...
                  <div className="act-display mt-5 text-3xl tracking-[-0.045em] text-brand-950">{count}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="overflow-hidden rounded-[24px] border border-[rgba(4,39,24,0.08)] bg-white lg:col-span-2">
          <div className="flex items-end justify-between border-b border-[rgba(4,39,24,0.08)] px-5 py-5 sm:px-6">
            <div>
              ...
              <h2 className="act-display mt-2 text-xl tracking-[-0.03em] text-brand-950">Top kandidat</h2>
            </div>
            <Link href="/company/candidates" className="text-xs font-semibold text-brand-600 hover:underline">Semua</Link>
          </div>
            ...
            <ul className="divide-y divide-[rgba(4,39,24,0.08)]">
              {topCandidates.map((candidate) => (
                <li key={candidate.id} className="flex items-center gap-3 px-5 py-3.5 sm:px-6">
                  <span className="inline-flex h-9 w-9 flex-none items-center justify-center rounded-full bg-brand-950 text-xs font-semibold text-white">
                    {candidate.name.split(" ").map((part) => part[0]).slice(0, 2).join("").toUpperCase()}
                  </span>
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-sm font-semibold text-brand-950">{candidate.name}</h3>
                    <p className="truncate text-xs text-[var(--act-graphite)]">{candidate.appliedFor}</p>
                  </div>
                  <span className="whitespace-nowrap rounded-full bg-[var(--act-wash-lilac)] px-2.5 py-1 text-[11px] font-bold text-brand-950">
