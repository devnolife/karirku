...
export default async function CompanyJobsPage() {
  ...
  return (
    <div className="act-rise mx-auto max-w-[1280px] space-y-8 px-5 py-8 sm:px-8 sm:py-12">
      <section className="overflow-hidden rounded-[24px] border border-[rgba(4,39,24,0.08)] bg-[#F2FBF6]">
        <div className="grid gap-8 px-6 py-7 sm:px-8 md:grid-cols-[minmax(0,1fr)_auto] md:items-end md:px-10 md:py-9">
          <div>
            <span className="text-xs font-bold uppercase tracking-[0.14em] text-[#198F38]">Hiring workspace / Portfolio</span>
            <h1 className="act-display mt-3 text-4xl text-[#042718] md:text-5xl">Lowongan yang sedang Anda bangun.</h1>
          ...
          <Link href="/company/jobs/new" className="act-pill shrink-0 !bg-[#042718] !px-5 !py-3 !text-sm">
            Posting lowongan
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-[0.14em] text-[#198F38]">Portfolio roles</span>
          <h2 className="act-heading mt-2 text-2xl text-[#042718]">Semua lowongan</h2>
        </div>
        <span className="rounded-full border border-[rgba(4,39,24,0.08)] bg-white px-3 py-1.5 text-xs font-semibold text-[#476655]">{jobs.length} peran</span>
      </div>
        ...
        <div className="overflow-hidden rounded-[24px] border border-[rgba(4,39,24,0.08)] bg-white shadow-[0_18px_45px_-36px_rgba(4,39,24,0.5)]">
          ...
          <ul className="divide-y divide-[rgba(4,39,24,0.08)]">
            {jobs.map((job) => {
              ...
              return (
                <li key={job.id} className="grid gap-4 px-5 py-5 transition-colors hover:bg-[#F2FBF6] md:grid-cols-[minmax(0,1.6fr)_0.8fr_0.9fr_0.7fr_auto] md:items-center md:gap-5 md:px-6">
                  <div className="min-w-0">
                    <p className="truncate text-[15px] font-semibold text-[#042718]">{job.title}</p>
                    <p className="mt-1 text-xs text-[#6E8D7B]">{job.location} <span aria-hidden>·</span> {job.posted}</p>
                  </div>
                  <div><span className="inline-flex rounded-full bg-[#F2FBF6] px-2.5 py-1 text-xs font-semibold text-[#315644]">{job.type}</span></div>
                  <div>
                    <p className="text-sm font-semibold text-[#042718]">{health}</p>
                    <div className="mt-2 h-1.5 max-w-[150px] overflow-hidden rounded-full bg-[#D4E5CD]">
                      <div className="h-full rounded-full bg-[#198F38]" style={{ width: `${Math.min(100, job.applicants * 20)}%` }} />
                    </div>
                  </div>
                  <div>
                    <span className={job.status === "active" ? "inline-flex rounded-full bg-[rgba(25,143,56,0.11)] px-2.5 py-1 text-xs font-bold text-[#136F2D]" : "inline-flex rounded-full bg-[#EBE3D2] px-2.5 py-1 text-xs font-bold text-[#765D2E]"}>
                      {job.status === "active" ? "Aktif" : "Draft"}
                    </span>
                  </div>
                  <div className="text-left md:text-right"><button className="text-xs font-bold text-[#198F38] transition-colors hover:text-[#042718] hover:underline">Edit</button></div>
                </li>
              );
            })}
          </ul>
...
function Metric({ label, value, note }: { label: string; value: number; note: string }) {
  return (
    <div className="border-t border-[rgba(4,39,24,0.08)] px-6 py-5 first:border-t-0 sm:border-l sm:border-t-0 sm:px-8 sm:first:border-l-0">
      ...
      <p className="act-display mt-1 text-3xl text-[#042718]">{value}</p>
